...
def solve():
    ...
    # it is guaranteed that all numbers for get_prime_divisors(q) will exist in get_prime_divisors(p)
    _count_q = cnt(get_prime_factors(q))

    ans = -INF

    for key, value in _count_q.items():
        new_p = p

        # remove until it is not divisible
        while new_p % key == 0:
            new_p //= key
