...
def solve():
    ...
    # if all positive, then we have to replace any two positives to negatives.
    if total == n:
        return total - 4

    # try to find two negatives. If we are able to find, then we will increase a total sum by 4
    for i in range(1, n):
        if a[i - 1] == a[i] and a[i] == -1:
            return total + 4

    # if not, then we leave sum unchanged.
    return total
